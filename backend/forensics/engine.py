...
def get_vessel_trajectory_data():
    """
    Retrieve timestamped AIS observations for every vessel.

    The frontend uses this data for:
    - real AIS trajectory rendering
    - time-aware playback
    ...
    with engine.connect() as connection:
        ...
        for row in result:
            ...
            trajectories[mmsi]["trajectory_path"].append(
                [
                    float(row.latitude),
                    float(row.longitude),
                ]
            ...
            trajectories[mmsi]["trajectory_points"].append(
                {
                    ...
                    "sog": (
                        ...
                        if row.speed_knots is not None
                        else None
                    ),
                    "cog": (
                        float(row.heading)
        ...
        for data in trajectories.values():
            points = data["trajectory_points"]

            if points:
                data["trajectory_start"] = (
                    points[0]["timestamp"]
                )
...
def run_forensic_analysis(
    ...
    dark_ship_data = get_dark_ship_evidence(
        dump_time
    )
    ...
    trajectory_data = get_vessel_trajectory_data()
    ...
    trajectory_by_mmsi = trajectory_data

    all_mmsi = (
        set(dark_by_mmsi)
        | set(spatial_by_mmsi)
        | set(movement_by_mmsi)
        | set(trajectory_by_mmsi)
    )
    ...
    for mmsi in all_mmsi:
        ...
        trajectory = trajectory_by_mmsi.get(mmsi)
        ...
        dark_ship_score = (
            calculate_dark_ship_score(
                gap_duration_minutes=dark[
                    "gap_duration_minutes"
                ],
                was_dark_at_dump=dark[
                    "was_dark_at_dump"
                ],
            )
            if dark
